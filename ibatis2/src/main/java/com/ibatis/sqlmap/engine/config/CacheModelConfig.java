...
public class CacheModelConfig {
	...
	// 构造时对CacheModel实例化对象赋初值
	CacheModelConfig(SqlMapConfiguration config, String id,
			CacheController controller, boolean readOnly, boolean serialize) {
		...
		try {
			...
			throw new RuntimeException(
					"Error setting Cache Controller Class.  Cause: " + e, e);
		}
	...
	public void setFlushInterval(long hours, long minutes, long seconds,
			long milliseconds) {
	...
	public void addFlushTriggerStatement(String statement) {
		errorContext
				.setMoreInfo("Check the cache model flush on statement elements.");
