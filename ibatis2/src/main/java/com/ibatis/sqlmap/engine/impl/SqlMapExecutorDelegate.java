...
//SqlMapExecutorDelegate是核心类、调度类或者是门面类。所有操作都要经过该类进行调度
public class SqlMapExecutorDelegate {

  private static final Probe PROBE = ProbeFactory.getProbe();

  private boolean lazyLoadingEnabled;//全局性地启用或禁用所有延迟加载。
  private boolean cacheModelsEnabled;//全局性地启用或禁用所有缓存model。
  private boolean enhancementEnabled;//全局性地启用或禁用运行时字节码增强，以优化访问Java Bean属性的性能，同时优化延迟加载的性能。
  private boolean useColumnLabel = true;
  private boolean forceMultipleResultSetSupport;

  private TransactionManager txManager;//用于事务处理

  private HashMap mappedStatements;  //用于映射语句处理的Map
  private HashMap cacheModels; //用于缓存处理的Map
  private HashMap resultMaps;  //用于输出结果的Map
  private HashMap parameterMaps; //用于输入参数的Map

  protected SqlExecutor sqlExecutor;  //最终的SQL执行体对象
  private TypeHandlerFactory typeHandlerFactory;  //数据类型转化的工厂对象
  private DataExchangeFactory dataExchangeFactory;  //数据交换的工厂对象
  
  private ResultObjectFactory resultObjectFactory;  //结果对象的工厂对象
  private boolean statementCacheEnabled; //全局性地启用或禁用SqlMapExecutorDelegate的statement是否可以缓存。
  ...
  public Object insert(SessionScope sessionScope, String id, Object param) throws SQLException {
    ...
    //获得新增操作的映射语句（MappedStatement）
    MappedStatement ms = getMappedStatement(id);
    
    // 获得新增操作的事务接口，主要是知道其事务状态
    ...
    try {
      ...
      //启动事务，然后执行操作，最后结束事务。当然如果已经启动了事务，那么启动和结束事务这两个方法失效
  ...
  public int update(SessionScope sessionScope, String id, Object param) throws SQLException {
    ...
    //获得修改操作的映射语句（MappedStatement）
    MappedStatement ms = getMappedStatement(id);
    
    // 获得修改操作的事务接口，主要是知道其事务状态
    Transaction trans = getTransaction(sessionScope);
    boolean autoStart = trans == null;
  ...
  public Object queryForObject(SessionScope sessionScope, String id, Object paramObject, Object resultObject) throws SQLException {
    ...
    //获得查询操作的映射语句（MappedStatement）
    MappedStatement ms = getMappedStatement(id);
    
    // 获得查询操作的事务接口，主要是知道其事务状态
    Transaction trans = getTransaction(sessionScope);
    boolean autoStart = trans == null;
  ...
  //进行查询操作，返回一个Map，实际上是获得一个列表，然后把列表转化为Map
  ...
  // 根据SessionScope对象中获得的事务对象，了解事务对象状态
