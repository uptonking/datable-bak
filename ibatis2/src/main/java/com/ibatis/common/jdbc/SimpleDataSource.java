...
public class SimpleDataSource implements DataSource {
            ...
            if (!(props.containsKey(PROP_JDBC_DRIVER)
                ...
                while (propIter.hasNext()) {
                    ...
                    if (name.startsWith(ADD_DRIVER_PROPS_PREFIX)) {
                        driverProps.put(name
                                        .substring(ADD_DRIVER_PROPS_PREFIX_LENGTH),
                                value);
                        useDriverProps = true;
                    }
                }
                ...
                // 根据JDBC驱动名称，实例化数据库驱动的对象，如同Class.forName(jdbcDriver);
                Resources.instantiate(jdbcDriver);

                if (poolPingEnabled
                        && (!props.containsKey(PROP_POOL_PING_QUERY) || prop_pool_ping_query
                        .trim().length() == 0)) {
                    throw new RuntimeException("SimpleDataSource: property '"
                            + PROP_POOL_PING_ENABLED
                            + "' is true, but property '"
    ...
    public long getAverageRequestTime() {
        synchronized (POOL_LOCK) {
            return requestCount == 0 ? 0 : accumulatedRequestTime
                    / requestCount;
        }
    }
    ...
    public long getAverageCheckoutTime() {
        synchronized (POOL_LOCK) {
            return requestCount == 0 ? 0 : accumulatedCheckoutTime
                    / requestCount;
        }
    }
    ...
    public void forceCloseAll() {
        ...
        if (log.isDebugEnabled()) {
            log
                    .debug("SimpleDataSource forcefully closed/removed all connections.");
        }
    }
    ...
    private void pushConnection(SimplePooledConnection conn)
        ...
        // 全部线程同步
        synchronized (POOL_LOCK) {
            //从活动连接池中删除一个SimplePooledConnection对象
            ...
            if (conn.isValid()) {
                ...
                if (idleConnections.size() < poolMaximumIdleConnections
                    ...
                    // 当可以获得新的SimplePooledConnection对象，解锁线程
                    POOL_LOCK.notifyAll();
                } else {
                    accumulatedCheckoutTime += conn.getCheckoutTime();
                    if (!conn.getRealConnection().getAutoCommit()) {
    ...
    private SimplePooledConnection popConnection(String username,
        ...
        while (conn == null) {
            ...
            synchronized (POOL_LOCK) {
                if (idleConnections.size() > 0) {
                    ...
                    // 在连接池是否有空闲的SimplePooledConnection实例化对象
                    ...
                    if (activeConnections.size() < poolMaximumActiveConnections) {
                        ...
                        // 不能创建一个新的SimplePooledConnection实例化对象

                        // 从活动连接池获取一个SimplePooledConnection实例化对象
                        SimplePooledConnection oldestActiveConnection = (SimplePooledConnection) activeConnections
                                .get(0);
                        long longestCheckoutTime = oldestActiveConnection
                                .getCheckoutTime();

                        //	判断SimplePooledConnection实例化对象是否超时
                ...
                if (conn != null) {
                    if (conn.isValid()) {
                        // 对于超时的、或者无效的SimplePooledConnection实例化对象，对于内部的数据库Connection进行回滚
                        if (!conn.getRealConnection().getAutoCommit()) {
                            conn.getRealConnection().rollback();
                        }
                        ...
                        if (localBadConnectionCount > (poolMaximumIdleConnections + 3)) {
                            if (log.isDebugEnabled()) {
                                log
                                        .debug("SimpleDataSource: Could not get a good connection to the database.");
                            }
        ...
        if (result) {
            if (poolPingEnabled) {
                if ((poolPingConnectionsOlderThan > 0 && conn.getAge() > poolPingConnectionsOlderThan)
                        || (poolPingConnectionsNotUsedFor > 0 && conn
    ...
    public static class SimplePooledConnection implements InvocationHandler {
        ...
        // 用于生成动态代理的Connection对象而定义的Connection类
        private static final Class[] IFACES = new Class[]{Connection.class};

        // 本SimplePooledConnection的哈希码
        private int hashCode = 0;

        //当前SimplePooledConnection对象的对应的SimpleDataSource实例化对象
        ...
        //判断当前SimplePooledConnection对象是否有效
