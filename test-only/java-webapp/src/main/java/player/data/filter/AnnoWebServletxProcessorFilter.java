package player.data.filter;

import player.data.anno.WebInitParamx;
 ...
 * 使用Filter作为注解的处理器
 */
public class AnnoWebServletxProcessorFilter implements Filter {

    private ServletContext servletContext = null;

    /* 过滤器初始化时扫描指定的包下面使用了WebServletx注解的那些类
    ...
    /**
     * 添加ServletClass到ServletContext中
     *
    ...
    private void addServletClassToServletContext(String packageName, Map<String, Class<?>> classMap) {
        ...
        for (Class<?> clazz : setClasses) {
            if (clazz.isAnnotationPresent(WebServletx.class)) {
                //获取WebServletx这个Annotation的实例
                WebServletx annotationInstance = clazz.getAnnotation(WebServletx.class);
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (uri.indexOf("!") == -1) {
            ...
            //获取要请求的servlet路径
            String requestServletName = dotIdx == -1 ? uri.substring(contextPath.length()) : uri.substring(contextPath.length(), dotIdx);
            //获取要使用的类
            ...
            try {
                //调用对象的方法进行处理
                // invoke前应该先 targetMethod.setAccessible(true); 不然会得到Access...Exception
            ...
            WebServletx annotationInstance = clazz.getAnnotation(WebServletx.class);
            //获取注解上配置的初始化参数数组
            WebInitParamx[] initParamArr = annotationInstance.initParams();
            Map<String, String> initParamMap = new HashMap<String, String>();
            for (WebInitParamx initParam : initParamArr) {
