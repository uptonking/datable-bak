...
public class StandardDaoManager implements DaoManager {
    ...
    //当前StandardDaoManager容器中所有的DaoContext对象，按照DaoContext的id来放置
    private Map idContextMap = new HashMap();

    //当前StandardDaoManager容器中所有的 (DaoImpl的接口， 对应的DaoContext对象)
    private Map typeContextMap = new HashMap();

    //当前StandardDaoManager容器中所有的DaoImpl对象(proxy对象/dao接口实例， DaoImpl实例对象)
    //数量是typeContextMap的双倍
