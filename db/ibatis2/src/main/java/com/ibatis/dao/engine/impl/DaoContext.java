...
public class DaoContext {
  ...
  private ThreadLocal transaction = new ThreadLocal(); // 处理事务的本地线程，主要实现线程操作
  private ThreadLocal state = new ThreadLocal();// 处理事务状态信息的本地线程
  private Map typeDaoImplMap = new HashMap();//当前DaoContext容器中所有的DaoImpl对象和DaoProxy对象
