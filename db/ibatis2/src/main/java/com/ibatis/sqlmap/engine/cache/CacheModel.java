...
public class CacheModel implements ExecuteListener {
  ...
  private String id; //缓存对象的唯一代码

  private boolean readOnly;  //缓存对象是否只读
  private boolean serialize;  //是否对缓存对象采用序列化操作

  private long lastFlush;  //最后一次刷新时间 
  private long flushInterval;   //刷新缓存的时间间隔，以微妙为单位
  private long flushIntervalSeconds;  //刷新缓存的间隔时间，以妙为单位
  private Set flushTriggerStatements;  //要进行缓存操作的方法

  private CacheController controller; //采用何种缓存控制器实现方式
